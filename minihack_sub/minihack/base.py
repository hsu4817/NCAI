...
class MiniHack(NetHackStaircase):
    ...
    def reset(self, stair, branch, *args, **kwargs):
        if self.reward_manager is not None:
            self.reward_manager.reset()
...
"""
        ...
        self.footer += f"STAIR:{stair},down\n"
        self.footer += f"BRANCH:{branch}\n"

       
        print(self.header + self.mapify(self.get_map_str()) + self.footer)
        des_file_2 = self.header + self.mapify(self.get_map_str()) + self.footer
        #************* end **************#
        self.update(self.des_file)
        
        self.update(des_file_2)
