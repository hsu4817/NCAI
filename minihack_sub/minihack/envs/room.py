...
from minihack.envs.location import location 
...
"""
This is JHJ's 5*5room for curriculum learning.
The start point and end point's distance changes to challenge.
So it means that the distance is bigger ... ><  >.<  >..<        
"""
import random

class MiniHackRoom_curi(MiniHackNavigation):
    """Environment for "empty" task."""
    ...
    def __init__(
        ...
        **kwargs
        ...
        """이거 다음주에 해야함!!!!"""
        # print("type: ",type(lvl_gen.get_des()))
        # print(lvl_gen.get_des())

        super().__init__(*args, des_file=lvl_gen.get_des(), **kwargs)



class MiniHackRoom5x5Random_curi(MiniHackRoom_curi):
    def __init__(self, *args, **kwargs):

        size = 5
        distance = 4
        start, end = location(distance, size)

        # print(start, end)

        super().__init__(*args, size = size, start = start, end = end, **kwargs)
...
register(
    ...
    entry_point="minihack.envs.room:MiniHackRoom5x5Random_curi",
)
register(
