    ...
    for name, team in config.teams.items():
        ...
        if ret == 0:
            ...
            if agent_path.exists():
                ...
                if len(name_agent_class) == 1:
                    agent_cls = tuple(name_agent_class.values())[0].strip()
                    agent_file = team.temp_dir / agent_cls
                else:
...
def play_games(config, run_start, run_end, verbose):
    ...
    for n in trange(run_start, run_end):
        tqdm.write(colored(f"Run: {n}", "green"))
        ...
        for team in tqdm(team_list, leave=False):
            ...
            tqdm.write(f" - Team: {team}, seed: {seed}")
            start = time.monotonic()
            try:
                result, log_buff, ttyrec = run_play_game(
                    agent,
                    env,
                    seed,
                    timeout,
                ...
                result, log_buff = {}, [f"{traceback.format_exc()}\n"]
...
def merge_result(config):
    shutil.copytree(
        config.data_dir, config.out_dir / config.data_dir.name, dirs_exist_ok=True
    )
